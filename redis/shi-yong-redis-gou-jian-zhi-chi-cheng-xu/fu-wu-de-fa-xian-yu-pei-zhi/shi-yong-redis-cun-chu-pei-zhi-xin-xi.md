...
为了实现这个简单的功能，让我们假设自己已经构建了一个中间层或者插件，这个中间层额作用在于：当is\__under\_maintenance\(\)函数返回True时，它将向用户显示维护页面；与此相反，如何\_is\_\_under\_maintenance\(\)函数返回False，它将如常地处理用户的访问请求。其中\_is\_\_under\_maintenance\(\)函数通过检查一个名为_is-_under-maintenance的键来判断服务器是否正在进行维护：如果_is-_under-maintenance_
